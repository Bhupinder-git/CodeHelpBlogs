  ...
  return (
    <NavLink className="block w-full" to={`/blogs/${id}`}>
      <div className="flex w-full min-w-0 justify-start gap-x-2 rounded-md border border-slate-300 bg-white p-2 shadow-md transition-all duration-600 hover:scale-103 hover:shadow-lg dark:border-slate-700 dark:bg-slate-900 md:gap-x-3 md:p-3">
        ...
        <div className="flex min-w-0 flex-1 flex-col items-start justify-center gap-1 font-body md:gap-2">
          {/* title - author - date */}
          <div>
            <h2 className="font-headline text-base font-bold text-slate-900 dark:text-slate-100 md:text-xl">
              {title}
            </h2>
          ...
          <div className="flex min-w-0 flex-col gap-y-2 md:gap-y-3">
            ...
            <div>
              {tags.map((tag, idx) => (
                <span
                  className="mx-1 text-sm text-blue-800 dark:text-blue-300 md:mx-2 md:text-base"
                  key={idx}
                >
                  #{tag}
                </span>
